...
export default function CreateStoryScreen({ navigation }) {
  ...
  const [slug, setSlug] = useState(""); // Slug sẽ lấy từ category được chọn
  ...
  const loadData = async () => {
    try {
      ...
      // Axios interceptor trả về response.data, nên authorRes = { success: true, data: authors }
      setAuthors(authorRes.data || authorRes.data?.data || []);
      // categoryRes = { success: true, data: categories }
