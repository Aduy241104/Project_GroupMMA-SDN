...
// 🔹 CREATE: Tạo chương mới (chỉ admin)
const createChapter = async (req, res, next) => {
    try {
        const { storyId, chapterNumber, title, content, images } = req.body;

        const story = await Story.findById(storyId);
        if (!story) return res.status(StatusCodes.NOT_FOUND).json({ 
            success: false,
            message: "Story not found" 
        });
        if (story.type !== "novel") return res.status(StatusCodes.BAD_REQUEST).json({ 
            success: false,
            message: "Only 'novel' stories can add chapters" 
        });

        // Kiểm tra trùng số chương và tiêu đề trong cùng story
        ...
        if (existingChapter) {
            return res.status(StatusCodes.BAD_REQUEST).json({
                success: false,
                message: "Chương với số chương và tiêu đề này đã tồn tại trong truyện này"
            });
        }
...
// 🔹 UPDATE: Cập nhật chương (chỉ admin)
const updateChapter = async (req, res, next) => {
    try {
        const { id } = req.params;
        const chapter = await Chapter.findById(id);
        if (!chapter) return res.status(StatusCodes.NOT_FOUND).json({ 
            success: false,
        ...
        if (story.type !== "novel") return res.status(StatusCodes.BAD_REQUEST).json({ 
            success: false,
            message: "Only 'novel' stories can update chapters" 
        });

        // Kiểm tra trùng số chương và tiêu đề (nếu thay đổi)
        const newChapterNumber = req.body.chapterNumber !== undefined ? req.body.chapterNumber : chapter.chapterNumber;
        const newTitle = req.body.title !== undefined ? req.body.title : chapter.title;
        
        if (newChapterNumber !== chapter.chapterNumber || newTitle !== chapter.title) {
            const existingChapter = await Chapter.findOne({ 
                storyId: chapter.storyId,
                chapterNumber: newChapterNumber,
                title: newTitle,
                _id: { $ne: id } // Loại trừ chính chapter đang sửa
            });
            if (existingChapter) {
                return res.status(StatusCodes.BAD_REQUEST).json({
                    success: false,
